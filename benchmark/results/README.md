...
| `native_rope_repeatkv.json` | Native RoPE dual-output and RepeatKV parity/latency, with version-3 memory plans |
| `native_kv_cache.json` | Two-step grouped-query attention cache parity and prefill/decode latency |
| `tinyllama_cached_cpu.json` | Offline full TinyLlama PyTorch CPU recompute versus populated KV-cache baseline |
| `tinyllama_leaf_coverage.json` | Reduced-width same-architecture ONNX operator-coverage probe; no trained weights loaded |
...
| `cifar10_resnet18_kv_no_regression.json` | Same-session alternating ResNet latency and parity gate after RoPE/RepeatKV/KV-cache changes |
